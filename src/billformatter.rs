...
pub trait BillFormatting {
    ...
    fn format_as_documentation(&self) -> Vec<Vec<String>> {
        self.list_of_user_ids()
            .iter()
            .flat_map(|id| self.format_as_personalized_documentation(id))
            .collect()
    }
...
impl OversightCSVLine {
    ...
    pub fn budget_outgoing(
        username: String,
        user_id: String,
        is_billed: bool,
        budget_cents_outgoing: i32,
        day: DateTime<Utc>,
        recipient: String,
        recipient_id: String,
    ) -> Self {
    ...
    pub fn budget_incoming(
        username: String,
        user_id: String,
        is_billed: bool,
        budget_cents_incoming: i32,
        day: DateTime<Utc>,
        donor: String,
        donor_id: String,
    ) -> Self {
    ...
    fn fmt(&self) -> Vec<String> {
        return vec![
            ...
            self.day.format(DATE_FORMAT_STRING).to_string(),
            self.item_name.to_string(),
            self.item_count.to_string(),
            cents_to_currency_string(self.item_cost_cents),
            self.budget_cents_outgoing.to_string(),
            self.donor.to_string(),
...
impl SewobeCSVLine {
    ...
    fn new(
        ...
        return SewobeCSVLine {
            ...
            bill_name: "Kantinenabrechnung ".to_string()
                + &billing_creation_date.format("%m/%y").to_string(),
            ...
            position_ends_date: billing_creation_date
                + time::Duration::seconds(100 * 365 * 24 * 60 * 60),
            tax_rate: "0".to_string(),
            description: "KA ".to_string()
                + &utc_timestamp_from
    ...
    fn fmt(&self) -> Vec<String> {
        vec![
            self.external_user_id.to_string(),
            if self.use_r_vs_g {
                "2".to_string()
            } else {
                "1".to_string()
            },
            self.bill_external_id.to_string(),
            self.bill_name.to_string(),
            self.bill_date.format(DATE_FORMAT_STRING).to_string(),
            ...
            self.payment_target_days.to_string(),
            self.sepa_interval.to_string(),
            self.bill_date_sent.format(DATE_FORMAT_STRING).to_string(),
            self.bill_date_late.format(DATE_FORMAT_STRING).to_string(),
            self.position_ends_date
                .format(DATE_FORMAT_STRING)
                .to_string(),
            self.tax_rate.to_string(),
            self.description.to_string(),
            if self.is_not_donation {
                "0".to_string()
            } else {
                "1".to_string()
            },
            self.donation_remark.to_string(),
            self.billkeeping_account.to_string(),
            self.tax_rate.to_string(),
            self.subaccount.to_string(),
        ]
    }
}
...
impl BillFormatting for Bill {
                ...
                && !self.users_that_will_not_be_billed.contains(user_id)
            {
                ...
                for day in &consumption.per_day.in_order_keys() {
                    ...
                    for item_id_purchase in &daycontent.personally_consumed.in_order_keys() {
                        let count = daycontent
                            .personally_consumed
                            .get(item_id_purchase)
                            .unwrap();
                        let item: rustix_bl::datastore::Item =
                            items.get(item_id_purchase).unwrap().clone();
                        result.push(
                            SewobeCSVLine::new(
                                timestamp_from,
                                timestamp_to,
                    ...
                    for item_id_ffa in &daycontent.ffa_giveouts.in_order_keys() {
                        ...
                        let item: rustix_bl::datastore::Item =
                            items.get(item_id_ffa).unwrap().clone();
                        result.push(
                            SewobeCSVLine::new(
                                timestamp_from,
                                timestamp_to,
                    ...
                    for other_user_id in &daycontent.giveouts_to_user_id.in_order_keys() {
                        ...
                        let other_user: rustix_bl::datastore::User =
                            users.get(other_user_id).unwrap().clone();
                        ...
                        for item_id in &paid_for.count_giveouts_used.in_order_keys() {
                            ...
                            let item: rustix_bl::datastore::Item =
                                items.get(&item_id).unwrap().clone();
                            result.push(
                                SewobeCSVLine::new(
                                    timestamp_from,
                                    timestamp_to,
    ...
    fn format_as_personalized_documentation(&self, user_id: &u32) -> Vec<Vec<String>> {
        ...
        //filter out unbilled user_ids
        let is_billed: bool = users.get(user_id).is_some()
            && users.get(user_id).unwrap().external_user_id.is_some()
            && users.get(user_id).unwrap().is_billed
            && !self.users_that_will_not_be_billed.contains(user_id);
        let consumption = self.finalized_data.user_consumption.get(user_id).unwrap();
        ...
        if users.get(user_id).is_some() && users.get(user_id).unwrap().external_user_id.is_some() {
            ...
            for day in &consumption.per_day.in_order_keys() {
                ...
                let day_timestamp: DateTime<Utc> = Utc.timestamp(timestamp_from / 1000, 0)
                    + time::Duration::seconds((60i64 * 60i64 * 24i64) * (*day as i64));
                ...
                for item_id_purchase in &daycontent.personally_consumed.in_order_keys() {
                    ...
                    let item: rustix_bl::datastore::Item =
                        items.get(item_id_purchase).unwrap().clone();
                    result.push(
                        OversightCSVLine::normal_purchase(
                            users.get(user_id).unwrap().username.to_string(),
                ...
                for special in &daycontent.specials_consumed {
                    result.push(
                        OversightCSVLine::special_purchase(
                            users.get(user_id).unwrap().username.to_string(),
                            external_user_id.to_string(),
                            is_billed,
                ...
                for other_user_id in &daycontent.giveouts_to_user_id.in_order_keys() {
                    ...
                    let other_user_id: String =
                        other_user.external_user_id.unwrap_or("".to_string());
                    ...
                    if budget_given > 0 {
                        result.push(
                            OversightCSVLine::budget_outgoing(
                                users.get(user_id).unwrap().username.to_string(),
                                external_user_id.to_string(),
                                is_billed,
                                budget_given as i32,
                                day_timestamp,
                                other_user_name.to_string(),
                    ...
                    if budget_gotten > 0 {
                        result.push(
                            OversightCSVLine::budget_incoming(
                                users.get(user_id).unwrap().username.to_string(),
                                external_user_id.to_string(),
                                is_billed,
                                budget_gotten as i32,
                                day_timestamp,
                                other_user_name.to_string(),
                                other_user_id.to_string(),
                            ).fmt(),
                    ...
                    for item_id in &paid_for.count_giveouts_used.in_order_keys() {
                        ...
                        result.push(
                            OversightCSVLine::count_giveout_outgoing(
                                users.get(user_id).unwrap().username.to_string(),
                                external_user_id.to_string(),
                                is_billed,
                                item.name,
...
mod tests {
    ...
    fn simple_sewobe_csv_works() {
            ...
            finalized_data: ExportableBillData {
                ...
                user_consumption: {
                    ...
                    consumption_map.insert(
                        0,
                        rustix_bl::datastore::BillUserInstance {
                            user_id: 0,
                            per_day: {
                                ...
                                day_hashmap.insert(
                                    day_index_1,
                                    rustix_bl::datastore::BillUserDayInstance {
                                        personally_consumed: {
                                ...
                                day_hashmap.insert(
                                    ...
                                    rustix_bl::datastore::BillUserDayInstance {
                                        ...
                                        specials_consumed: vec![
                                            rustix_bl::datastore::PricedSpecial {
        ...
        let should: Vec<Vec<String>> = vec![
            ...
            vec![
                ...
                "Anzahl",
                "Preis pro Einheit",
                "2 == Lastschrift und 1 == Ueberweisung",
                "Empfang per Mail",
                "Zahlungsziel in Tagen",
                "SEPA Intervall (0 fuer einmalig)",
                "Datum Rechnungsstellung",
                "Datum Faelligkeit",
                "Datum Positionsende",
                "Mehrwertsteuersatz",
                "Beschreibung",
    ...
    fn date_format_works() {
        let day_timestamp: chrono::DateTime<chrono::Utc> =
            chrono::Utc.timestamp(1523794067, 347000);
        assert_eq!(
            "15.04.2018".to_string(),
            day_timestamp.format(DATE_FORMAT_STRING).to_string()
        )
    }
