...
fn enrich_purchase(
    incoming: &rustix_bl::datastore::Purchase,
    datastore: &rustix_bl::datastore::Datastore,
) -> std::result::Result<Purchase, Box<std::error::Error>> {
...
fn enrich_freeby(
    incoming: &rustix_bl::datastore::Freeby,
    datastore: &rustix_bl::datastore::Datastore,
) -> std::result::Result<EnrichedCountOrBudgetGiveout, Box<std::error::Error>> {
    return match *incoming {
        rustix_bl::datastore::Freeby::Transfer {
            ref id,
            ...
            Ok(EnrichedCountOrBudgetGiveout {
                ...
                cents_worth_left: *cents_worth_total - *cents_worth_used,
                total: 0,
                left: 0,
        ...
        rustix_bl::datastore::Freeby::Classic {
            ref id,
            ...
            for (_, it) in &datastore.items {
                if (!it.deleted)
                    && (ids.contains(&it.item_id)
                        || (it.category.is_some() && cats.contains(&it.category.clone().unwrap())))
                {
                    items.push(it.clone());
                }
            }
...
fn enrich_ffa(
    incoming: &rustix_bl::datastore::Freeby,
    datastore: &rustix_bl::datastore::Datastore,
) -> std::result::Result<EnrichedFFA, Box<std::error::Error>> {
    return match *incoming {
        rustix_bl::datastore::Freeby::FFA {
            ref id,
            ref allowed_categories,
            ref allowed_drinks,
            ref allowed_number_total,
...
pub trait ServableRustix {
    ...
    */
    fn check_apply_write(
        backend: &mut Backend,
        app_state: ParametersAll,
        write_event: rustix_bl::rustix_event_shop::BLEvents,
    ) -> Result<RefreshedData, Box<::std::error::Error>>;
}
    ...
    fn query_read(
        ...
        match query {
            AllItems(param) => {
                let xs = backend
                    .datastore
                    .items_searchhit_ids(&param.count_pars.searchterm);
                ...
                let result: PaginatedResult<rustix_bl::datastore::Item> = PaginatedResult {
                    ...
                    results: v.iter()
                        .take(param.pagination.end_exclusive as usize)
                        .skip(param.pagination.start_inclusive as usize)
                        .map(|r| r.clone())
                        .collect(),
                };
            ...
            AllUsers(param) => {
                let xs = backend
                    .datastore
                    .users_searchhit_ids(&param.count_pars.searchterm);

                debug!(
                    "AllUsersQuery with total store =\n{:?}\nvs\n{:?}",
            ...
            TopUsers(param) => {
                ...
                let highlight_users: HashSet<u32> = backend
                    .datastore
                    .highlighted_users
                    .iter()
                    .map(|c| *c)
                ...
                for uid in xs_full {
                    if !highlight_users.contains(&uid)
                        && (param.n - highlighted - (xs.len() as u16)) > 0
                    {
                        xs.push(uid);
                    }
                }
            ...
            DetailInfoForUser(param) => {
                ...
                let bill = backend
                    ...
                    .bills
                    .iter()
                    .filter(|b| b.bill_state.is_finalized())
                    .last();

                let xs =
                    backend
                        .datastore
                        .personal_log_filtered(param.user_id, three_months_ago, now);
                ...
                for y in &backend
                    .datastore
                    .personal_log_filtered(param.user_id, 0, now)
                {
                ...
                for x in xs {
                    if backend.datastore.users.get(x.get_user_id()).is_some() && x.has_item_id()
                        && backend.datastore.items.get(x.get_item_id()).is_some()
                    {
                        let itemname = backend
                            .datastore
                            .users
                            .get(x.get_user_id())
                            .unwrap()
            ...
            TopPersonalDrinks(param) => {
                ...
                let v: Vec<rustix_bl::datastore::Item> = xs.iter()
                    .map(|id| backend.datastore.items.get(id).unwrap().clone())
                    .collect();
            ...
            PurchaseLogGlobal(param) => {
                let mut xs = backend
                    .datastore
                    .global_log_filtered(param.count_pars.millis_start, param.count_pars.millis_end)
                    .to_vec();
            ...
            PurchaseLogPersonal(param) => {
                let mut xs = backend.datastore.personal_log_filtered(
                    param.count_pars.user_id,
                    param.count_pars.millis_start,
                    param.count_pars.millis_end,
                );
            ...
            Bills(param) => {
                let mut xs = backend
                    .datastore
                    .bills_filtered(
                        param.count_pars.scope_user_id,
                        param.count_pars.start_inclusive,
                        param.count_pars.end_exclusive,
                    )
                    .to_vec();
            ...
            BillDetails(param) => {
                let result = if param.timestamp_from.is_some() && param.timestamp_to.is_some() {
                    ...
                    let mut users_excludable_but_not_internally_indices: Vec<
                        usize,
                    > = vec![];
                    ...
                    let xs = vec![DetailedBill {
                        timestamp_from: ts_from,
                        timestamp_to: ts_to,
                        bill_state: bill.bill_state,
                        comment: bill.comment,
                        users: bill.users,
                        ready_for_finalization: unset_specials_indices.is_empty()
                            && users_undefined_indices.is_empty(),
                        ...
                        users_excludable_but_not_internally_indices:
                            users_excludable_but_not_internally_indices,
                    }];
            ...
            AllUsersCount(_param) => panic!("Not supported"),
            AllItemsCount(_param) => panic!("Not supported"),
            PurchaseLogGlobalCount(_param) => panic!("Not supported"),
            BillsCount(_param) => panic!("Not supported"),
            OpenFFAFreebies(param) => {
                let xs: Vec<Freeby> = backend
                    .datastore
                    .open_ffa
                    .iter()
                    .map(|x| x.clone())
            ...
            PurchaseLogPersonalCount(_param) => panic!("Not supported"),
            IncomingFreebiesCount(_param) => panic!("Not supported"),
            IncomingFreebies(param) => {
                let xsopt = backend
                    .datastore
                    .open_freebies
                    .get(&param.count_pars.recipient_id);
            ...
            OutgoingFreebiesCount(_param) => panic!("Not supported"),
    ...
    fn check_apply_write(
        ...
        match write_event {
            ...
            rustix_event_shop::BLEvents::UpdateUser {
                ...
                let changed_bill_details =
                    Self::query_read(&*backend, BillDetails(app_state.bill_detail_infos))?;
            ...
            rustix_event_shop::BLEvents::MakeSimplePurchase {
                ...
                let top_items =
                    Self::query_read(&*backend, TopPersonalDrinks(app_state.top_personal_drinks))?;
                //refresh detailinfo for user
                let detail_info = Self::query_read(
                    &*backend,
                    DetailInfoForUser(app_state.personal_detail_infos),
                )?;
                //refresh global log
                let global_log =
                    Self::query_read(&*backend, PurchaseLogGlobal(app_state.global_log))?;
                //refresh last log
                let last_log = Self::query_read(
                    &*backend,
                    PurchaseLogGlobal(ParametersPurchaseLogGlobal {
                        count_pars: ParametersPurchaseLogGlobalCount {
                            millis_start: server::current_time_millis() - (1000i64 * 60 * 60 * 24),
                            millis_end: server::current_time_millis() + 1000i64,
                        },
                ...
                let personal_log =
                    Self::query_read(&*backend, PurchaseLogPersonal(app_state.personal_log))?;
            ...
            a @ rustix_event_shop::BLEvents::MakeFreeForAllPurchase { .. } => {
                ...
                let open_ffa =
                    Self::query_read(&*backend, OpenFFAFreebies(app_state.open_ffa_freebies))?;
            ...
            a @ rustix_event_shop::BLEvents::CreateFreeCount { .. } => {
                ...
                let incoming =
                    Self::query_read(&*backend, IncomingFreebies(app_state.incoming_freebies))?;
                let outgoing =
                    Self::query_read(&*backend, OutgoingFreebies(app_state.outgoing_freebies))?;
                Ok(RefreshedData {
                    DetailInfoForUser: serde_json::Value::Null,
                    TopUsers: serde_json::Value::Null,
                    AllUsers: serde_json::Value::Null,
            ...
            a @ rustix_event_shop::BLEvents::FinalizeBill { .. } => {
                ...
                let outgoing =
                    Self::query_read(&*backend, OutgoingFreebies(app_state.outgoing_freebies))?;
                Ok(RefreshedData {
                    DetailInfoForUser: Self::query_read(
                        &*backend,
                        DetailInfoForUser(app_state.personal_detail_infos),
                    )?,
                    TopUsers: serde_json::Value::Null,
                    AllUsers: serde_json::Value::Null,
                    AllItems: serde_json::Value::Null,
                    PurchaseLogGlobal: Self::query_read(
                        &*backend,
                        PurchaseLogGlobal(app_state.global_log),
                    )?,
                    LastPurchases: last_log,
                    BillsCount: serde_json::Value::Null,
                    Bills: bills,
                    ...
                    OpenFFAFreebies: serde_json::Value::Null,
                    TopPersonalDrinks: serde_json::Value::Null,
                    PurchaseLogPersonal: Self::query_read(
                        &*backend,
                        PurchaseLogPersonal(app_state.personal_log),
                    )?,
                    IncomingFreebies: incoming,
                    OutgoingFreebies: outgoing,
                })
...
pub fn fill_backend_with_large_test_data(backend: &mut Backend) -> () {
    ...
    let _ = (*back).create_ffa(
        vec!["Liquor".to_string()],
        vec![0, 1, 2],
        3,
        "I'm donating this to present how this would work in the current cervisia ui".to_string(),
        timestamp_counter + 1,
        0,
    );
    ...
    (*back).create_bill(
        0i64,
        timestamp_counter + (1000i64 * 3600 * 24 * 365 * 20),
        UserGroup::AllUsers,
        "some bill comment".to_string(),
    );
}
